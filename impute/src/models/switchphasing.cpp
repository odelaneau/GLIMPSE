...
switchphasing::switchphasing(haplotype_set * _H, conditioning_set * _C) : H(_H), C(_C) {
}

switchphasing::~switchphasing(){}

void switchphasing::sampling(std::vector < bool > & H0, std::vector < bool > & H1) {
	...
	forwardProbs = std::vector < std::vector < float > > (2, std::vector < float > (C->n_states, 0.0));
	prevForwardProbs = std::vector < std::vector < float > > (2, std::vector < float > (C->n_states, 0.0));
	...
	std::vector < float > alphaSumCurrH = std::vector < float > (4, 0.0);
	std::vector < float > prevAlphaSumCurrH = std::vector < float > (4, 0.0);
	for (int l = 0 ; l < C->n_sites ; l++) {
		...
		if (H0[l] == H1[l]) {
			...
			if (l == 0) {
				...
				for (int k = 0 ; k < C->n_states ; k ++) {
					forwardProbs[0][k] = fact1[0] * (HAP[0][l]==H->H_opt_var.get(l,C->idxH[k]))?C->ee:C->ed;
					forwardProbs[1][k] = fact1[1] * (HAP[1][l]==H->H_opt_var.get(l,C->idxH[k]))?C->ee:C->ed;
			...
			*/
			...
			if (l == 0) {
				...
				for (int k = 0 ; k < C->n_states ; k ++) {
					...
					//forwardProbs[2][k] = fact1[0] * ((HAP[1][l]==H->H_opt_var.get(l,C->idxH[k]))?C->ee:C->ed);	//t2
					//forwardProbs[3][k] = fact1[1] * ((HAP[0][l]==H->H_opt_var.get(l,C->idxH[k]))?C->ee:C->ed);	//t3
				...
				for (int k = 0 ; k < C->n_states ; k ++) {
					...
					forwardProbs[0][k] = (prevForwardProbs[0][k]  * fact2[0] + fact1[0]) * ((HAP[0][l]==H->H_opt_var.get(l,C->idxH[k]))?C->ee:C->ed);	//f0 + a0
					forwardProbs[1][k] = (prevForwardProbs[1][k]  * fact2[1] + fact1[1]) * ((HAP[1][l]==H->H_opt_var.get(l,C->idxH[k]))?C->ee:C->ed);	//f1 + a1
					for (int c = 0 ; c < 2 ; c++) alphaSumCurrH[c] += forwardProbs[c][k];
					//forwardProbs[2][k] = (prevForwardProbs[0][k]  * fact2[0] + fact1[0]) * ((HAP[1][l]==H->H_opt_var.get(l,C->idxH[k]))?C->ee:C->ed);	//f0 + a1
					//forwardProbs[3][k] = (prevForwardProbs[1][k]  * fact2[1] + fact1[1]) * ((HAP[0][l]==H->H_opt_var.get(l,C->idxH[k]))?C->ee:C->ed);	//f1 + a0
					//for (int c = 0 ; c < 4 ; c++) alphaSumCurrH[c] += forwardProbs[c][k];
				}
			}
			//PHASE
			...
			//cout << l << " " << (H0[l] == H1[l])  << " " << stb.str(phasingD[0], 3) << " " << stb.str(phasingD[1], 3) << " " << stb.str(phasingD[2], 3) << " " << stb.str(phasingD[3], 3) << endl;
